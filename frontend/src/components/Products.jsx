...
export default function Products() {
  ...
  const categories = [
    { id: "waterConditioner", name: t('products.categories.waterConditioner'), icon: Droplets, color: "from-teal-500 to-cyan-500" },
    { id: "tankHanger", name: t('products.categories.tankHanger'), icon: Filter, color: "from-emerald-500 to-teal-500" },
    { id: "purifier", name: t('products.categories.purifiers'), icon: Settings, color: "from-purple-500 to-indigo-500" },
  ];
  ...
  const waterConditioners = useMemo(() => [
    { id: 1, model: "5810", size: '1"', price: "27,800", tag: t('products.tags.popular'), flow: "500 LPH" },
    { id: 2, model: "5815", size: '1.1/4" - 1.1/2"', price: "33,500", tag: t('products.tags.bestValue'), flow: "800 LPH" },
    ...
    { id: 6, model: "5840", size: '4"', price: "56,500", tag: t('products.tags.commercial'), flow: "3000 LPH" },
    { id: 7, model: "5850", size: '5"', price: "98,000", tag: t('products.tags.industrial'), flow: "5000 LPH" },
    { id: 8, model: "5860", size: '6"', price: "1,45,000", tag: t('products.tags.heavyDuty'), flow: "8000 LPH" },
  ], [t]);

  // Tank Hanger - Single product
  const tankHangerProducts = useMemo(() => [
    { 
      id: 1, 
      name: t('products.tankHanger.name'), 
      price: "32,000", 
      tag: t('products.tags.premiumQuality'), 
      image: "/products/tank-hanger/standard.jpeg",
      images: ["/products/tank-hanger/standard.jpeg"],
      features: [
        t('products.tankHanger.feature1'),
        t('products.tankHanger.feature2'),
        t('products.tankHanger.feature3'),
        t('products.tankHanger.feature4'),
        t('products.tankHanger.feature5')
      ]
    }
  ], [t]);
  ...
  const purifierBrands = useMemo(() => [
    ...
    { id: 6, name: t('products.purifiers.customOrder'), model: t('products.purifiers.customModel'), price: null, capacity: "Custom", type: "Commercial", rating: 5, images: ["/products/purifiers/custom.jpg", "/products/purifiers/custom-2.jpg"] },
  ], [t]);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 relative z-10">
        ...
        <div className="text-center mb-8 sm:mb-10">
          <div className={`inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-secondary-500/10 border border-secondary-500/20 mb-3 sm:mb-4 fade-up ${visible ? "visible" : ""}`}>
            ...
            <span className="text-secondary-500 font-poppins text-xs sm:text-sm font-medium">{t('products.premiumBadge')}</span>
          </div>
          <h1 className={`font-poppins font-bold text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-text-light-primary dark:text-text-dark-primary mb-2 sm:mb-3 fade-up ${visible ? "visible" : ""}`}>
            UB'CAL <span className="text-secondary-500">{t('products.waterConditioner')}</span>
          </h1>
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <span className="xs:hidden">{cat.id === "waterConditioner" ? t('products.shortConditioner') : cat.id === "waterPurifiers" ? t('products.shortPurifiers') : cat.name}</span>
              </button>
            );
          })}
        </div>
              ...
              {imgError ? (
                  ...
                  <div className="text-center px-4">
                    <p className="font-semibold text-text-light-primary dark:text-text-dark-primary text-xs sm:text-sm">
                      {activeCategory === "purifier" ? selectedItem?.name :
                       ...
                       `${t('products.model')} ${selectedItem?.model}`}
                    </p>
                  </div>
                </div>
              ) : (
                <img
                  ...
                  alt={activeCategory === "purifier" ? `${selectedItem?.name} ${selectedItem?.model}` :
                       activeCategory === "tankHanger" ? selectedItem?.name :
                       `UB'CAL ${t('products.model')} ${selectedItem?.model}`}
                ...
                <div className="absolute bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 sm:gap-2">
                  {[...Array(Math.min(imageCount, 8))].map((_, idx) => (
                    <button
                      ...
                      aria-label={`${t('products.goToImage')} ${idx + 1}`}
                    />
                  ))}
                </div>
              )}
              ...
              {selectedItem?.tag && (
                <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
                  <span className={`text-[10px] sm:text-xs px-2 sm:px-3 py-1 sm:py-1.5 rounded-full font-bold shadow-lg ${
                    selectedItem.tag === t('products.tags.popular') ? "bg-amber-500" :
                    selectedItem.tag === t('products.tags.bestValue') ? "bg-emerald-500" :
                    selectedItem.tag === t('products.tags.commercial') ? "bg-blue-500" :
                    selectedItem.tag === t('products.tags.industrial') ? "bg-orange-500" :
                    selectedItem.tag === t('products.tags.heavyDuty') ? "bg-red-500" :
                    selectedItem.tag === t('products.tags.premiumQuality') ? "bg-purple-500" :
                    "bg-secondary-500"
                  } text-white`}>
                    {selectedItem.tag}
                  </span>
          ...
          <div className="space-y-4 sm:space-y-5">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-text-light-primary dark:text-text-dark-primary mb-1 sm:mb-2">
                {activeCategory === "waterConditioner" ? t('products.waterConditionerTitle') :
                 activeCategory === "tankHanger" ? t('products.tankHangerTitle') :
                 t('products.purifiersTitle')}
              </h2>
              <p className="text-text-light-secondary dark:text-text-dark-secondary text-xs sm:text-sm">
                {activeCategory === "waterConditioner" ? t('products.selectPipeSize') :
                 activeCategory === "tankHanger" ? t('products.tankHangerDesc') :
            ...
            <div className="space-y-2 sm:space-y-3">
              <label className="block text-xs sm:text-sm font-medium text-text-light-primary dark:text-text-dark-primary">
                {activeCategory === "waterConditioner" ? t('products.selectPipeSize') : activeCategory === "tankHanger" ? t('products.selectModel') : t('products.selectBrand')}
              </label>
                  ...
                  >
                    {activeCategory === "waterConditioner" && (
                      <>
                        ...
                        <div className="text-[10px] sm:text-xs opacity-80">{t('products.model')} {item.model}</div>
                      </>
                    )}
            ...
            <div className={`rounded-2xl p-4 sm:p-6 ${
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-text-light-secondary">{t('products.yourPrice')}</p>
                  <div className="text-3xl sm:text-4xl font-bold text-secondary-500">
                    {selectedItem?.price ? `₹${selectedItem.price}` : t('products.contactUs')}
                  </div>
                  {selectedItem?.price && <p className="text-[10px] sm:text-xs text-text-light-muted mt-0.5 sm:mt-1">{t('products.gst')}</p>}
                </div>
            ...
            {/* Tank Hanger Additional Details */}
            ...
            <div className="flex flex-col xs:flex-row gap-3 sm:gap-4">
              ...
              >
                <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {t('products.callNow')}
              </a>
            </div>
          </div>
        </div>
